...
public record UpdateLinkAction(Type type, Link link) implements Action {
    ...
    public Result<Path, Code> apply(FileSystemReader fsReader, FileSystemWriter fsWriter) {
        ...
        try {
            ...
            return Result.error(new Code(Code.State.ERROR, "Unable to update link " + e.getMessage(), previousLink));
        }
    }
}
